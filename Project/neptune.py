...
def get_neptune_distance():
    """
    Fetch real-time distance data for neptune from Earth using NASA's JPL Horizons API
    ...
    try:
        ...
        # Query Horizons API (299 is neptune's NAIF ID, 399 is Earth)
        target = Horizons(id='899', location='399', epochs=current_jd)
        ...
        json_path = os.path.join(data_dir, 'neptune_distance.json')
        with open(json_path, 'w') as f:
            json.dump(result, f)
            
        print(f"neptune distance data updated: {distance_au:.6f} AU ({distance_km:,.2f} km)")
        ...
        print(f"Error fetching neptune distance data: {str(e)}")
        return {
            "distance_au": None,
            "distance_km": None,
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
...
if __name__ == "__main__":
    # Get and print neptune distance data
    data = get_neptune_distance()
    
    if data["distance_au"] is not None:
        print("\nReal-time distance from Earth to neptune:")
        print(f"{data['distance_au']:.6f} AU")
        print(f"{data['distance_km']:,.2f} km")
        print(f"Data timestamp: {data['timestamp']}")
    else:
        print(f"Failed to retrieve neptune distance data: {data.get('error', 'Unknown error')}")
