...
def get_saturn_distance():
    """
    Fetch real-time distance data for saturn from Earth using NASA's JPL Horizons API
    ...
    try:
        ...
        # Query Horizons API (299 is saturn's NAIF ID, 399 is Earth)
        target = Horizons(id='699', location='399', epochs=current_jd)
        ...
        json_path = os.path.join(data_dir, 'saturn_distance.json')
        with open(json_path, 'w') as f:
            json.dump(result, f)
            
        print(f"saturn distance data updated: {distance_au:.6f} AU ({distance_km:,.2f} km)")
        ...
        print(f"Error fetching saturn distance data: {str(e)}")
        return {
            "distance_au": None,
            "distance_km": None,
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
...
if __name__ == "__main__":
    # Get and print saturn distance data
    data = get_saturn_distance()
    
    if data["distance_au"] is not None:
        print("\nReal-time distance from Earth to saturn:")
        print(f"{data['distance_au']:.6f} AU")
        print(f"{data['distance_km']:,.2f} km")
        print(f"Data timestamp: {data['timestamp']}")
    else:
        print(f"Failed to retrieve saturn distance data: {data.get('error', 'Unknown error')}")
