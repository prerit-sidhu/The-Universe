...
def get_mars_distance():
    """
    Fetch real-time distance data for mars from Earth using NASA's JPL Horizons API
    ...
    try:
        ...
        # Query Horizons API (299 is mars's NAIF ID, 399 is Earth)
        target = Horizons(id='499', location='399', epochs=current_jd)
        ...
        json_path = os.path.join(data_dir, 'mars_distance.json')
        with open(json_path, 'w') as f:
            json.dump(result, f)
            
        print(f"mars distance data updated: {distance_au:.6f} AU ({distance_km:,.2f} km)")
        ...
        print(f"Error fetching mars distance data: {str(e)}")
        return {
            "distance_au": None,
            "distance_km": None,
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
...
if __name__ == "__main__":
    # Get and print mars distance data
    data = get_mars_distance()
    
    if data["distance_au"] is not None:
        print("\nReal-time distance from Earth to mars:")
        print(f"{data['distance_au']:.6f} AU")
        print(f"{data['distance_km']:,.2f} km")
        print(f"Data timestamp: {data['timestamp']}")
    else:
        print(f"Failed to retrieve mars distance data: {data.get('error', 'Unknown error')}")
